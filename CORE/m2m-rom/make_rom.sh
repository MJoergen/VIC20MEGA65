...
##############################################################################
# VIC 20 specific: Generate osm_const.asm
##############################################################################
...

{
    ...
    echo ""
    echo "; -----------------------------------------------------------------"
    echo "; VIC 20 specific OSM item constants"
    echo "; -----------------------------------------------------------------"
    echo ""
} > osm_const.asm

awk '/constant C_MENU_/ {gsub("C_MENU_", "VIC20_OSM_"); gsub(";", ""); printf("%-27s .EQU %s\n", $2, $6)}' ../vhdl/mega65.vhd >> osm_const.asm

{
    echo ""
    echo "; -----------------------------------------------------------------"
    echo "; VIC 20 specific menu group items"
    echo "; -----------------------------------------------------------------"
    echo ""
} >> osm_const.asm

awk '/constant OPTM_G_/ && !/16#/ {gsub("OPTM_G_", "VIC20_OPTM_G_"); gsub(";", ""); printf("%-27s .EQU %s\n", $2, $6)}' ../vhdl/config.vhd >> osm_const.asm
